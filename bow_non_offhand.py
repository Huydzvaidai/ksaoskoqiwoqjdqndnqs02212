...
import json
import glob
from bow_non_offhand_utils import Bow_non_offhand_Util
from item_non_offhand_utils import Item_non_offhand_Util
...
Bow_non_offhand_Util.rendercontrollers()
gmdllist = []
for file in files:
    try:
        ...
        if data["check"] == 4:
            ...
            for i in range(4):
                ...
                with open(fa, "r") as f:
                    ...
                    is2Dbow =  Bow_non_offhand_Util.is2Dbow(glob.glob(f"staging/target/rp/models/entity/{namespace}/{path}.json")[0])
                    if is2Dbow:
                    ...
                    if i == 0:
                        ...
                        Bow_non_offhand_Util.item_texture(gmdl, textures[0])
                    else:
                        os.remove(fa)
            # Only process 3D bows
            if not is2Dbow:
                Bow_non_offhand_Util.write(mfile, gmdl, textures, geometry, mdefault, menchanted, animations, animate, pre_animation, display_data)
...
Bow_non_offhand_Util.merge_bow_models()
Bow_non_offhand_Util.acontroller(gmdllist)
