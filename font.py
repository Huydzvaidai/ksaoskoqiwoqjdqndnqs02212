...
parser = argparse.ArgumentParser(description='Generate font sprites with exclusion option')
parser.add_argument('--exclude-patterns', nargs='+', default=['_iainternal:hud/small_cooldown_bar/', '_iainternal:gui/', 'char/'],
                    help='Patterns to exclude from processing (default: _iainternal:hud/small_cooldown_bar/ _iainternal:gui/ char/)')
...
for d in data['providers']:
    try:
        ...
        for pattern in args.exclude_patterns:
            if re.search(re.escape(pattern.rstrip('/')), file_path):
                should_skip = True
                break
