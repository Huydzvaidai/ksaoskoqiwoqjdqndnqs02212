#!/usr/bin/env python3
"""
Java to Bedrock Resource Pack Converter - Fast Python Implementation
Replaces converter.sh completely
"""

import argparse
import hashlib
import json
import os
import shutil
import subprocess
import sys
import zipfile
import uuid
import random
import string
from pathlib import Path
from concurrent.futures import ProcessPoolExecutor, ThreadPoolExecutor, as_completed
from typing import Dict, List, Optional, Tuple, Any
import urllib.request

# ============================================================================
# UTILITIES
# ============================================================================

class Colors:
    RED = '\033[31m'
    GREEN = '\033[32m'
    YELLOW = '\033[33m'
    BLUE = '\033[36m'
    GRAY = '\033[37m'
    CLOSE = '\033[0m'

def status_message(msg_type: str, message: str):
    """Print colored status messages"""
    if msg_type == "completion":
        print(f"{Colors.GREEN}[+] {Colors.GRAY}{message}{Colors.CLOSE}")
    elif msg_type == "process":
        print(f"{Colors.YELLOW}[•] {Colors.GRAY}{message}{Colors.CLOSE}")
    elif msg_type == "critical":
        print(f"{Colors.RED}[X] {Colors.GRAY}{message}{Colors.CLOSE}")
    elif msg_type == "error":
        print(f"{Colors.RED}[ERROR] {Colors.GRAY}{message}{Colors.CLOSE}")
    elif msg_type == "info":
        print(f"{Colors.BLUE}{message}{Colors.CLOSE}")
    else:
        print(f"{Colors.GRAY}{message}{Colors.CLOSE}")

def load_json(filepath: str) -> Optional[Dict[str, Any]]:
    """Load JSON file safely"""
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            return json.load(f)
    except:
        return None

def save_json(filepath: str, data: Dict[str, Any], compact: bool = True):
    """Save JSON file"""
    os.makedirs(os.path.dirname(filepath) or '.', exist_ok=True)
    with open(filepath, 'w', encoding='utf-8') as f:
        if compact:
            json.dump(data, f, separators=(',', ':'), ensure_ascii=False)
        else:
            json.dump(data, f, indent=2, ensure_ascii=False)

def md5_hash(text: str, length: int = 7) -> str:
    """Generate MD5 hash"""
    return hashlib.md5(text.encode()).hexdigest()[:length]

def generate_uuid() -> str:
    """Generate UUID"""
    return str(uuid.uuid4())

def generate_random_name(length: int = 20) -> str:
    """Generate random alphanumeric name"""
    return ''.join(random.choices(string.ascii_letters + string.digits, k=length))

def resolve_namespace(path: str) -> str:
    """Extract namespace from path"""
    return path.split(':')[0] if ':' in path else 'minecraft'

def resolve_model_path(parent: str) -> str:
    """Convert model reference to file path"""
    ns = resolve_namespace(parent)
    model = parent.split(':')[1] if ':' in parent else parent
    return f"./assets/{ns}/models/{model}.json"

def resolve_texture_path(texture: str, namespace: str = 'minecraft') -> str:
    """Convert texture reference to file path"""
    ns = resolve_namespace(texture)
    tex = texture.split(':')[1] if ':' in texture else texture
    return f"./assets/{ns}/textures/{tex}.png"

def download_file(url: str, output: str):
    """Download file"""
    try:
        urllib.request.urlretrieve(url, output)
    except Exception as e:
        status_message("error", f"Failed to download: {e}")
        raise

def extract_zip(zip_path: str, extract_to: str = "."):
    """Extract zip file"""
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        zip_ref.extractall(extract_to)

def create_zip(source_dir: str, output_path: str):
    """Create zip file from directory"""
    with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk(source_dir):
            for file in files:
                if file.startswith('.'):
                    continue
                file_path = os.path.join(root, file)
                arcname = os.path.relpath(file_path, source_dir)
                zipf.write(file_path, arcname)

# ============================================================================
# CONFIG GENERATOR
# ============================================================================

def generate_config(item_models_dir: str, item_texture_file: str, item_mappings_file: str) -> Dict[str, Any]:
    """Generate config.json from item models"""
    status_message("process", "Generating initial predicate config")
    
    item_texture = load_json(item_texture_file) or {}
    item_mappings = load_json(item_mappings_file) or {}
    
    # Build max damage lookup
    max_damage = {}
    for key, value in item_mappings.items():
        if isinstance(value, dict) and 'max_damage' in value:
            item_name = key.split(':')[1] if ':' in key else key
            max_damage[item_name] = value['max_damage']
    
    config = {}
    counter = 1
    
    item_models_path = Path(item_models_dir)
    if not item_models_path.exists():
        return {}
    
    for json_file in item_models_path.glob('*.json'):
        if json_file.name == 'potion.json':
            continue
        
        item_name = json_file.stem
        data = load_json(str(json_file))
        
        if not data or 'overrides' not in data:
            continue
        
        for override in data['overrides']:
            predicate = override.get('predicate', {})
            model = override.get('model', '')
            
            if not any(k in predicate for k in ['custom_model_data', 'damage', 'damaged']):
                continue
            
            nbt = {}
            
            if 'damage' in predicate:
                max_dur = max_damage.get(item_name, 1)
                nbt['Damage'] = int(predicate['damage'] * max_dur + 0.5)
            
            if 'damaged' in predicate and predicate['damaged'] == 0:
                nbt['Unbreakable'] = True
            
            if 'custom_model_data' in predicate:
                nbt['CustomModelData'] = predicate['custom_model_data']
            
            bedrock_icon = item_texture.get(item_name, {"icon": "camera", "frame": 0})
            
            ns = resolve_namespace(model)
            model_clean = model.split(':')[1] if ':' in model else model
            model_parts = model_clean.split('/')
            model_file = model_parts[-1]
            model_dir = '/'.join(model_parts[:-1]) if len(model_parts) > 1 else ''
            
            geyser_id = f"campfire_{counter}"
            
            predicate_str = f"{item_name}_c{nbt.get('CustomModelData', '')}_d{nbt.get('Damage', '')}_u{nbt.get('Unbreakable', '')}"
            path_str = f"./assets/{ns}/models/{model_clean}.json"
            entry_hash = md5_hash(predicate_str)
            path_hash = md5_hash(path_str)
            
            entry = {
                "item": item_name,
                "bedrock_icon": bedrock_icon,
                "nbt": nbt,
                "path": path_str,
                "namespace": ns,
                "model_path": model_dir,
                "model_name": model_file,
                "generated": False,
                "geyserID": geyser_id,
                "path_hash": f"campfire_{entry_hash}",
                "geometry": f"campfiregeo_{path_hash}"
            }
            
            config[geyser_id] = entry
            counter += 1
    
    # Filter valid entries and add parent info
    valid_config = {}
    for gid, entry in config.items():
        if Path(entry['path']).exists():
            model_data = load_json(entry['path'])
            if model_data and 'parent' in model_data:
                entry['parent'] = model_data['parent']
            valid_config[gid] = entry
    
    status_message("completion", f"Config generated with {len(valid_config)} entries")
    return valid_config

# ============================================================================
# PARENTAL RESOLVER
# ============================================================================

def resolve_parental_chain(file_path: str, max_depth: int = 20) -> Tuple[Optional[dict], Optional[dict], Optional[dict]]:
    """Resolve parental chain"""
    elements = None
    textures = None
    display = None
    current_file = file_path
    depth = 0
    
    while depth < max_depth:
        data = load_json(current_file)
        if not data:
            break
        
        if elements is None and 'elements' in data:
            elements = data['elements']
        
        if textures is None and 'textures' in data:
            textures = data['textures']
        
        if display is None and 'display' in data:
            display = data['display']
        
        if elements is not None and textures is not None and display is not None:
            break
        
        parent = data.get('parent')
        if not parent or parent in ['builtin/generated', 'minecraft:builtin/generated']:
            break
        
        current_file = resolve_model_path(parent)
        depth += 1
    
    return elements, textures, display

def process_single_parented_model(args: Tuple) -> Tuple[str, str, Optional[str], Optional[str]]:
    """Process single parented model"""
    file_path, gid, parental, namespace, model_path, model_name, path_hash = args
    
    try:
        current_data = load_json(file_path)
        if not current_data:
            return gid, 'deleted', None, f"Failed to load {file_path}"
        
        elements = current_data.get('elements')
        textures = current_data.get('textures')
        display = current_data.get('display')
        
        if elements is None or textures is None or display is None:
            p_elements, p_textures, p_display = resolve_parental_chain(file_path)
            
            if elements is None:
                elements = p_elements
            if textures is None:
                textures = p_textures
            if display is None:
                display = p_display
        
        parent = current_data.get('parent', '')
        is_generated = parent in ['builtin/generated', 'minecraft:builtin/generated']
        
        if is_generated and textures and elements is None:
            texture_0 = None
            if isinstance(textures, dict):
                first_key = list(textures.keys())[0] if textures else None
                if first_key:
                    texture_ref = textures[first_key]
                    texture_0 = resolve_texture_path(texture_ref, namespace)
            
            if texture_0 and os.path.exists(texture_0):
                resolved = {'textures': textures}
                if display:
                    resolved['display'] = display
                save_json(file_path, resolved)
                
                target_dir = f"./target/rp/textures/{namespace}"
                if model_path:
                    target_dir = f"{target_dir}/{model_path}"
                os.makedirs(target_dir, exist_ok=True)
                
                target_file = f"{target_dir}/{model_name}.png"
                shutil.copy2(texture_0, target_file)
                
                if model_path:
                    icon_path = f"textures/{namespace}/{model_path}/{model_name}"
                else:
                    icon_path = f"textures/{namespace}/{model_name}"
                icon_path = icon_path.replace('//', '/')
                icon_entry = f"{path_hash},{icon_path}"
                
                return gid, 'generated', icon_entry, None
            else:
                return gid, 'deleted', None, f"Texture not found"
        
        if elements is not None and textures is not None:
            resolved = {'textures': textures, 'elements': elements}
            if display is not None:
                resolved['display'] = display
            
            save_json(file_path, resolved)
            return gid, 'success', None, None
        
        return gid, 'deleted', None, f"No valid data"
    
    except Exception as e:
        return gid, 'deleted', None, str(e)

def process_parented_models(config: Dict, workers: int = None) -> Dict:
    """Process all parented models"""
    if workers is None:
        workers = max(1, os.cpu_count() - 1)
    
    tasks = []
    for gid, entry in config.items():
        if 'parent' in entry:
            tasks.append((
                entry['path'],
                gid,
                entry['parent'],
                entry['namespace'],
                entry['model_path'],
                entry['model_name'],
                entry['path_hash']
            ))
    
    if not tasks:
        return config
    
    status_message("process", f"Processing {len(tasks)} parented models")
    
    results = {}
    icons = []
    generated = []
    deleted = []
    completed = 0
    
    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(process_single_parented_model, task): task for task in tasks}
        
        for future in as_completed(futures):
            gid, status, icon_entry, error = future.result()
            results[gid] = status
            completed += 1
            
            if status == 'generated':
                generated.append(gid)
                if icon_entry:
                    icons.append(icon_entry)
            elif status == 'deleted':
                deleted.append(gid)
            
            if completed % 10 == 0 or completed == len(tasks):
                progress = (completed * 100) // len(tasks)
                print(f"\rProgress: {completed}/{len(tasks)} ({progress}%)", end='', flush=True)
    
    print()
    
    # Update config
    for gid in generated:
        if gid in config:
            config[gid]['generated'] = True
    
    for gid in deleted:
        config.pop(gid, None)
    
    # Save icon mappings
    if icons:
        os.makedirs("scratch_files", exist_ok=True)
        with open("scratch_files/icons.csv", 'w') as f:
            f.write('\n'.join(icons))
    
    status_message("completion", f"Processed: {len(results) - len(deleted)} successful, {len(deleted)} deleted")
    
    return config

# ============================================================================
# MAIN CONVERTER CLASS
# ============================================================================

class Converter:
    def __init__(self, args):
        self.args = args
        self.config = {}
        self.scratch_dir = Path("scratch_files")
        self.target_rp = Path("target/rp")
        self.target_bp = Path("target/bp")
        
    def run(self):
        """Main conversion process"""
        try:
            self.setup()
            self.extract_input_pack()
            
            if not self.check_valid_pack():
                return
            
            # Check for early exit
            if not Path("assets/minecraft/models/item").exists():
                self.early_exit_path()
                return
            
            self.download_mappings()
            self.generate_config_file()
            self.setup_directories()
            
            if self.args.fallback_pack != 'none':
                self.download_fallback_resources()
            
            self.process_models()
            self.run_manager()
            self.package_output()
            
            status_message("completion", "Conversion complete!")
            
        except Exception as e:
            status_message("error", f"Conversion failed: {e}")
            import traceback
            traceback.print_exc()
            sys.exit(1)
    
    def setup(self):
        """Setup directories"""
        self.scratch_dir.mkdir(exist_ok=True)
    
    def extract_input_pack(self):
        """Extract input pack"""
        status_message("process", "Decompressing input pack")
        extract_zip(self.args.input_pack)
        
        potion_path = Path("assets/minecraft/models/item/potion.json")
        if potion_path.exists():
            potion_path.unlink()
            status_message("completion", "Removed potion.json")
        
        status_message("completion", "Input pack decompressed")
    
    def check_valid_pack(self) -> bool:
        """Check if pack.mcmeta exists"""
        if not Path("pack.mcmeta").exists():
            status_message("error", "Invalid resource pack! pack.mcmeta not found")
            return False
        return True
    
    def early_exit_path(self):
        """Handle packs without item models"""
        status_message("process", "No item models found, creating minimal pack")
        self.setup_directories()
        
        # Run manager.py
        subprocess.run([sys.executable, "manager.py"], capture_output=True)
        
        # Package
        self.package_output()
        
        status_message("completion", "Minimal pack created")
    
    def download_mappings(self):
        """Download Geyser mappings"""
        status_message("process", "Downloading Geyser mappings")
        
        mappings_url = "https://raw.githubusercontent.com/GeyserMC/mappings/master/items.json"
        texture_url = "https://raw.githubusercontent.com/Kas-tle/java2bedrockMappings/main/item_texture.json"
        
        download_file(mappings_url, str(self.scratch_dir / "item_mappings.json"))
        download_file(texture_url, str(self.scratch_dir / "item_texture.json"))
        
        status_message("completion", "Mappings downloaded")
    
    def generate_config_file(self):
        """Generate config.json"""
        self.config = generate_config(
            "assets/minecraft/models/item",
            str(self.scratch_dir / "item_texture.json"),
            str(self.scratch_dir / "item_mappings.json")
        )
        
        save_json("config.json", self.config, compact=False)
    
    def setup_directories(self):
        """Create directory structure"""
        status_message("process", "Creating directory structure")
        
        dirs = [
            self.target_rp / "models/entity",
            self.target_rp / "textures",
            self.target_rp / "textures/layer_armor",
            self.target_rp / "attachables",
            self.target_rp / "animations",
            self.target_bp / "blocks",
            self.target_bp / "items"
        ]
        
        for d in dirs:
            d.mkdir(parents=True, exist_ok=True)
        
        # Copy pack icon
        if Path("pack.png").exists():
            shutil.copy("pack.png", self.target_rp / "pack_icon.png")
            shutil.copy("pack.png", self.target_bp / "pack_icon.png")
        
        # Generate manifests
        uuid1 = generate_uuid()
        uuid2 = generate_uuid()
        uuid3 = generate_uuid()
        uuid4 = generate_uuid()
        
        pack_desc = "§fPack create by §a§lKaito §rDiscord §a@_jaysonmalon"
        
        rp_manifest = {
            "format_version": 2,
            "header": {
                "description": "Made By Kaito",
                "name": pack_desc,
                "uuid": uuid1,
                "version": [3, 0, 0],
                "min_engine_version": [1, 21, 0]
            },
            "modules": [{
                "description": "Made By Kaito",
                "type": "resources",
                "uuid": uuid2,
                "version": [3, 0, 0]
            }]
        }
        save_json(str(self.target_rp / "manifest.json"), rp_manifest)
        
        bp_manifest = {
            "format_version": 2,
            "header": {
                "description": "Adds 3D items for use with a Geyser proxy",
                "name": pack_desc,
                "uuid": uuid3,
                "version": [1, 0, 0],
                "min_engine_version": [1, 18, 3]
            },
            "modules": [{
                "description": "Adds 3D items for use with a Geyser proxy",
                "type": "data",
                "uuid": uuid4,
                "version": [1, 0, 0]
            }],
            "dependencies": [{
                "uuid": uuid1,
                "version": [1, 0, 0]
            }]
        }
        save_json(str(self.target_bp / "manifest.json"), bp_manifest)
        
        # Texture definitions
        terrain_texture = {
            "resource_pack_name": "geyser_custom",
            "texture_name": "atlas.terrain",
            "texture_data": {}
        }
        save_json(str(self.target_rp / "textures/terrain_texture.json"), terrain_texture)
        
        item_texture = {
            "resource_pack_name": "geyser_custom",
            "texture_name": "atlas.items",
            "texture_data": {}
        }
        save_json(str(self.target_rp / "textures/item_texture.json"), item_texture)
        
        # Disable animation
        disable_anim = {
            "format_version": "1.8.0",
            "animations": {
                "animation.campfire.disable": {
                    "loop": True,
                    "override_previous_animation": True,
                    "bones": {
                        "campfire": {
                            "scale": 0
                        }
                    }
                }
            }
        }
        save_json(str(self.target_rp / "animations/animation.campfire.disable.json"), disable_anim)
        
        status_message("completion", "Directory structure created")
    
    def download_fallback_resources(self):
        """Download fallback resources"""
        status_message("process", "Downloading fallback resources")
        
        version = self.args.default_asset_version or "1.21"
        url = f"https://github.com/InventivetalentDev/minecraft-assets/zipball/refs/tags/{version}"
        
        if not Path("default_assets.zip").exists():
            download_file(url, "default_assets.zip")
        
        # Extract and merge
        extract_zip("default_assets.zip", "defaultassetholding")
        
        root_folders = list(Path("defaultassetholding").iterdir())
        if root_folders:
            root_folder = root_folders[0]
            
            src_textures = root_folder / "assets/minecraft/textures"
            src_models = root_folder / "assets/minecraft/models"
            
            dst_textures = Path("assets/minecraft/textures")
            dst_models = Path("assets/minecraft/models")
            
            if src_textures.exists():
                dst_textures.mkdir(parents=True, exist_ok=True)
                for item in src_textures.rglob('*'):
                    if item.is_file():
                        rel_path = item.relative_to(src_textures)
                        dst_file = dst_textures / rel_path
                        if not dst_file.exists():
                            dst_file.parent.mkdir(parents=True, exist_ok=True)
                            shutil.copy2(item, dst_file)
            
            if src_models.exists():
                dst_models.mkdir(parents=True, exist_ok=True)
                for item in src_models.rglob('*'):
                    if item.is_file():
                        rel_path = item.relative_to(src_models)
                        dst_file = dst_models / rel_path
                        if not dst_file.exists():
                            dst_file.parent.mkdir(parents=True, exist_ok=True)
                            shutil.copy2(item, dst_file)
        
        shutil.rmtree("defaultassetholding", ignore_errors=True)
        
        status_message("completion", "Fallback resources merged")
    
    def process_models(self):
        """Process parented models"""
        self.config = process_parented_models(self.config)
        
        # Update config.json
        save_json("config.json", self.config, compact=False)
        
        # Update item_texture.json with icons
        icons_csv = Path("scratch_files/icons.csv")
        if icons_csv.exists():
            item_texture_path = self.target_rp / "textures/item_texture.json"
            item_texture = load_json(str(item_texture_path)) or {"texture_data": {}}
            
            with open(icons_csv, 'r') as f:
                for line in f:
                    parts = line.strip().split(',')
                    if len(parts) == 2:
                        path_hash, texture_path = parts
                        item_texture['texture_data'][path_hash] = {
                            "textures": texture_path.replace('//', '/')
                        }
            
            save_json(str(item_texture_path), item_texture)
    
    def run_manager(self):
        """Run manager.py"""
        status_message("process", "Running conversion manager")
        
        result = subprocess.run([sys.executable, "manager.py"], capture_output=True, text=True)
        
        if result.returncode != 0:
            status_message("error", f"Manager failed: {result.stderr}")
    
    def package_output(self):
        """Package output packs"""
        status_message("process", "Packaging output")
        
        # Move target to bedrock
        if Path("target").exists():
            if Path("bedrock").exists():
                shutil.rmtree("bedrock")
            shutil.move("target", "bedrock")
        
        # Flatten structure
        if Path("bedrock/rp").exists():
            for item in Path("bedrock/rp").iterdir():
                shutil.move(str(item), "bedrock/")
            Path("bedrock/rp").rmdir()
        
        if Path("bedrock/bp").exists():
            shutil.rmtree("bedrock/bp")
        
        # Minify JSON
        for json_file in Path("bedrock").rglob("*.json"):
            data = load_json(str(json_file))
            if data:
                save_json(str(json_file), data, compact=True)
        
        # Create bedrock.mcpack
        create_zip("bedrock", "bedrock.mcpack")
        
        status_message("completion", "Output packaged")

# ============================================================================
# MAIN ENTRY POINT
# ============================================================================

def main():
    parser = argparse.ArgumentParser(description='Java to Bedrock Resource Pack Converter')
    parser.add_argument('input_pack', help='Input Java resource pack (zip file)')
    parser.add_argument('-w', '--warn', default='true', help='Show warning message')
    parser.add_argument('-m', '--merge_input', default='null', help='Existing bedrock pack to merge')
    parser.add_argument('-a', '--attachable_material', default='entity_alphatest_one_sided', help='Attachable material')
    parser.add_argument('-b', '--block_material', default='alpha_test', help='Block material')
    parser.add_argument('-f', '--fallback_pack', default='null', help='Fallback pack URL')
    parser.add_argument('-v', '--default_asset_version', default='1.21', help='Default asset version')
    parser.add_argument('-r', '--rename_model_files', default='false', help='Rename model files')
    parser.add_argument('-s', '--save_scratch', default='false', help='Save scratch files')
    parser.add_argument('-u', '--disable_ulimit', default='false', help='Disable ulimit')
    
    args = parser.parse_args()
    
    # Check input pack exists
    if not os.path.exists(args.input_pack):
        status_message("error", f"Input pack {args.input_pack} not found")
        sys.exit(1)
    
    # Show warning
    if args.warn != 'false':
        print(f"{Colors.RED}")
        print("=" * 79)
        print(" " * 20 + "# <!> # W A R N I N G # <!> #")
        print("=" * 79)
        print("This script has been provided as is. If your resource pack does not")
        print("entirely conform the vanilla resource specification, there is a")
        print("strong possibility this script will fail.")
        print("=" * 79)
        print(f"{Colors.CLOSE}")
        input("Press Enter to continue or Ctrl+C to exit...")
    
    # Run converter
    converter = Converter(args)
    converter.run()

if __name__ == "__main__":
    main()
