...
class Crossbow_non_offhand_Util:
    @staticmethod
    def get_crossbow_variants_info(namespace, model_path=""):
        ...
        # Lấy geometry identifiers với fallback
        default_geometry = get_geometry_from_model(crossbow_model) if crossbow_model else None
        pulling_1_geometry = get_geometry_from_model(crossbow_1_model) if crossbow_1_model else default_geometry
        pulling_2_geometry = get_geometry_from_model(crossbow_charged_model) if crossbow_charged_model else default_geometry
    ...
    def get_display_initialize(display_data, namespace=None, model_path="", model_name="crossbow"):
        ...
        # Lấy crossbow charged display data
        crossbow_charged_data = Crossbow_non_offhand_Util.get_crossbow_charged_display_data(namespace, model_path, model_name)
    ...
    def update_attachable(file, identifier, materials, textures, geometry, animations, display_data=None):
        ...
        initialize = Crossbow_non_offhand_Util.get_display_initialize(display_data, namespace, model_path, model_name)
