...
class Item_non_offhand_Util:
    ...
    def get_display_initialize(display_data):
        ...
        return [
            ...
            "v.thirdperson_offhand_rot_x = 0;",
            "v.thirdperson_offhand_rot_y = 0;",
            "v.thirdperson_offhand_rot_z = 0;",
            "v.thirdperson_offhand_pos_x = 0;",
            "v.thirdperson_offhand_pos_y = 0;",
            "v.thirdperson_offhand_pos_z = 0;",
            "v.thirdperson_offhand_scale = 0;",
            ...
            "v.firstperson_offhand_rot_x = 0;",
            "v.firstperson_offhand_rot_y = 0;",
            "v.firstperson_offhand_rot_z = 0;",
            "v.firstperson_offhand_pos_x = 0;",
            "v.firstperson_offhand_pos_y = 0;",
            "v.firstperson_offhand_pos_z = 0;",
            "v.firstperson_offhand_scale = 0;",
    ...
    def update_attachable(file, identifier, materials, textures, geometry, animations, display_data=None):
        ...
        initialize = Item_non_offhand_Util.get_display_initialize(display_data)
    ...
    def create_2d_geometry_file():
        file = "staging/target/rp/models/entity/tool_custom.json"
        os.makedirs(os.path.dirname(file), exist_ok=True)
...
# Chạy tự động khi module được import
if __name__ == "__main__":
    # Tạo các file cần thiết
    Item_non_offhand_Util.animations()
    Item_non_offhand_Util.create_2d_animation_file()
    Item_non_offhand_Util.create_2d_geometry_file()
else:
    # Nếu được import từ module khác, vẫn chạy
    Item_non_offhand_Util.animations()
    Item_non_offhand_Util.create_2d_animation_file()
    Item_non_offhand_Util.create_2d_geometry_file()
...
for attachable_file in attachable_files:
    try:
        ...
        # Bỏ qua bow attachables (bow.py sẽ xử lý riêng)
        ...
        # Tìm model file tương ứng
        model_pattern = f"staging/target/rp/models/entity/{namespace}/**/*.json"
        model_files = glob.glob(model_pattern, recursive=True)
        
        # Tìm model file có tên tương tự
        model_file = None
        attachable_name = os.path.splitext(os.path.basename(attachable_file))[0]
        
        for mf in model_files:
            model_name = os.path.splitext(os.path.basename(mf))[0]
            if model_name == attachable_name:
                model_file = mf
                break
        
        # Đọc display data từ model gốc nếu có
        display_data = {}
        if model_file and os.path.exists(model_file):
            try:
                with open(model_file, "r") as f:
                    model_data = json.load(f)
                    display_data = model_data.get("display", {})
            except:
                pass
        
        # Kiểm tra xem có phải 2D item không
        is_2d_item = Item_non_offhand_Util.is_2d_item(geometry)
        
        # Xử lý theo loại item (tự động phát hiện)
        if is_2d_item:
            # Cập nhật attachable cho 2D item
            Item_non_offhand_Util.update_attachable_2d(
                attachable_file,
                identifier,
                materials,
                textures,
            ...
            # Cập nhật attachable với initialize section mới cho 3D item
            Item_non_offhand_Util.update_attachable(
                attachable_file,
                identifier,
                materials,
                textures,
